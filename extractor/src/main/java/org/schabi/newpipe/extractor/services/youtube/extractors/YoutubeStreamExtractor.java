...
public class YoutubeStreamExtractor extends StreamExtractor {
    ...
    public String getName() throws ParsingException {
        ...
        try {
            JsonObject jsonObject = getVideoPrimaryInfoRenderer();
            if (jsonObject != null) {
                if (jsonObject.has("title")) {
                    title = getTextFromObject(jsonObject.getObject("title"));
                } else {
                    title = "unknow";
                }
            } else {
    ...
    public String getVideoId() throws ParsingException {
        assertPageFetched();
        try {
            return playerResponse.getObject("videoDetails").getString("videoId");
        } catch (final Exception e) {
    ...
    @Override
    public String getVideostatsPlaybackUrl() throws ParsingException {
        assertPageFetched();
        try {
            return playerResponse.getObject("playbackTracking").getObject("videostatsPlaybackUrl").getString("baseUrl");
        } catch (final Exception e) {
            throw new ParsingException("Could not get videostats PlaybackUrl");
        }
    }
    ...
    @Override
    public boolean isLike() throws ParsingException {
        assertPageFetched();
        JsonArray topLevelButtons;
        boolean isLike = false;

        try {
            topLevelButtons = getVideoPrimaryInfoRenderer().getObject("videoActions").getObject("menuRenderer").getArray("topLevelButtons");
            for (final Object topLevelButton : topLevelButtons) {
                JsonObject toggleButtonRenderer = ((JsonObject) topLevelButton).getObject("toggleButtonRenderer");
                String targetId = toggleButtonRenderer.getString("targetId");
                if (targetId.equals("watch-like")) {
                    isLike = toggleButtonRenderer.getBoolean("isToggled");
                    break;
                }
            }
        } catch (final Exception e) {
            throw new ParsingException("Could not get like status", e);
        }
    ...
    public boolean isSubscribed() throws ParsingException {
        assertPageFetched();
        JsonObject subscribeButtonRenderer;
        boolean isSubscribed = false;

        try {
            subscribeButtonRenderer = getVideoSecondaryInfoRenderer().getObject("subscribeButton").getObject("subscribeButtonRenderer");
            isSubscribed = subscribeButtonRenderer.getBoolean("subscribed");
        } catch (final Exception e) {
            throw new ParsingException("Could not get subscribe status", e);
        }
    ...
    @Override
    public boolean isDisLike() throws ParsingException {
        assertPageFetched();
        JsonArray topLevelButtons;
        ...
        try {
            ...
            for (final Object topLevelButton : topLevelButtons) {
                JsonObject toggleButtonRenderer = ((JsonObject) topLevelButton).getObject("toggleButtonRenderer");
                String targetId = toggleButtonRenderer.getString("targetId");
                if (targetId.equals("watch-dislike")) {
                    isDisLike = toggleButtonRenderer.getBoolean("isToggled");
                    break;
                }
            }
        } catch (final Exception e) {
            throw new ParsingException("Could not get dislike status", e);
        }
    ...
    public StreamInfoItemsCollector getRelatedItems() throws ExtractionException {
        ...
        try {
            ...
            for (final Object ul : results) {
                if (((JsonObject) ul).has("itemSectionRenderer")) {
                    JsonArray contents = ((JsonObject) ul).getObject("itemSectionRenderer").getArray("contents");
                    for (final Object content: contents) {
                        if (((JsonObject) content).has("compactVideoRenderer")) {
                            collector.commit(new YoutubeStreamInfoItemExtractor(((JsonObject) content)
                                    .getObject("compactVideoRenderer"), timeAgoParser));
                        }
                    }
                } else  if (((JsonObject) ul).has("compactVideoRenderer")) {
    ...
    public void onFetchPage(@Nonnull final Downloader downloader)
        ...
        NewPipe.parseLoginStatus(playerResponse);
        ...
        NewPipe.parseLoginStatus(nextResponse);
...
//            throw e;
    ...
    private void fetchDesktopJsonPlayerWithSts(final ContentCountry contentCountry,
        ...
        try {
            JsonObject playerResponseWithSignatureTimestamp = getJsonPostResponse(
    ...
    private JsonObject getVideoPrimaryInfoRenderer() throws ParsingException {
        ...
        try {
            contents = nextResponse.getObject("contents")
                    .getObject("twoColumnWatchNextResults").getObject("results").getObject("results")
                    .getArray("contents");
        } catch (Exception e) {
            throw new ParsingException("nextResponse is null");
        }
